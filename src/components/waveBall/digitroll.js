...
export default class DigitRoll {
    constructor(opts) {
      this.container = document.querySelector(opts.container) // 容器
      let height = opts.height || 120
      this.width = opts.width || 1 
      if (!this.container) {
          throw Error('no container') 
      }
      this.container.style.overflow = 'hidden' 
      this.rollHeight = parseInt(getComputedStyle(this.container).height, 10) // 容器高度 也用于滚动间隔距离
...
DigitRoll.prototype = {
    ...
    roll:function (n) {
        ...
        Array.prototype.forEach.call(this.container.querySelectorAll('.num'), function (item,i) {
            var currentNum = parseInt(item.querySelector('div:last-child').innerHTML, 10); // 当前数字
            var goalNum = parseInt(self.number[i], 10); // 目标数字
            ...
            if (currentNum == goalNum) { // 数字没变 不处理
                ...
                for (var j2 = currentNum; j2 < 10; j2++) {
                    gapStr += '<div>' + j2 + '</div>'
                }
                for (var x = 0; x < goalNum + 1; x++) {
                    gapStr += '<div>' + x + '</div>'
                }
            }
